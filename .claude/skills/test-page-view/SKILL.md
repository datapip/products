...
Report a concise pass/fail summary in chat for both test cases (opt-in, opt-out), listing each assertion from steps 3.5 and 6.5 individually with ✅/❌ — including the Google Ads checks. Since Google Ads traffic hasn't been validated by this skill before, if its request shape differs from what step 3 describes, report it as a flagged finding with what was actually observed rather than a bare ❌. Also include the paths to the four saved artifact files. Do not write a separate report file — the artifact files plus the chat summary are the deliverable.
